...
def decrypt_aes_cbc_with_ecb(text, key, iv): 
	'''
	>>> key = bytes([57, 226, 240, 61, 125, 240, 75, 68, 22, 35, 124, 205, 144, 27, 118, 220])
	...
	for i in range(0, len(text), 16):
		chunk = text[i : i + 16]
		decrypted += xor_byte_arrays(decrypt_aes_ecb(chunk, key), prev_chunk)
		prev_chunk = chunk
	return bytes(decrypted)

def encrypt_aes_cbc_with_ecb(text, key, iv):
	'''
	>>> key = bytes([57, 226, 240, 61, 125, 240, 75, 68, 22, 35, 124, 205, 144, 27, 118, 220])
	>>> iv = bytes([241, 147, 66, 129, 194, 34, 37, 51, 236, 69, 188, 205, 64, 140, 244, 204])
	...
	for i in range(0, len(text), 16):
		chunk = text[i : i + 16]
		encrypted_block = encrypt_aes_ecb(xor_byte_arrays(chunk, prev_chunk), key)
