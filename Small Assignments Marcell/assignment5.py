...
def bit_permutation(bits, order):
	'''
	...
	new = []
	for curr in order: 
		new.append(bits[curr-1])
	return ''.join(new)

def left_shift_rot(bits, step=1):
	'''
	>>> left_shift_rot('010')
	'100'
	>>> left_shift_rot('111')
	...
	'1000'
	'''
	return bits[step :] + bits[0 : step]

def PKCS7_pad(string, to_pad):
	'''
	>>> PKCS7_pad('hello',6)
	'hello\\x01'
	>>> PKCS7_pad('hello',7)
	...
	'''
	diff = to_pad - len(string)
	pad =  str(chr(diff).encode('utf-8'))[2:-1] * diff
	return string + pad
