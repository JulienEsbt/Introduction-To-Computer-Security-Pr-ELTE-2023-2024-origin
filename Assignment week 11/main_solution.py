...
def hack_sha256_fixed_size(target, repeat):
    '''
    ...
    'elte'
    '''
    for c in itertools.product(chars,repeat=repeat):
        text = "".join(c)
        if sha256(text) == target:
            return text
    return None

def hack_sha256(target):
    '''
    ...
    '''
    for i in range(10):
        result = hack_sha256_fixed_size(target, i)
...
def authenticate_with_pepper(username, password):
    ...
    False
    '''
    return username in users_with_pepper and sha256(pepper_prefix+password) == users_with_pepper[username]['passwordHash']
...
def authenticate_with_pepper_and_salt(username, password):
    ...
    False
    '''
    return username in users_with_pepper and sha256(pepper_prefix+password+users_with_pepper_and_salt[username]['salt']) == users_with_pepper_and_salt[username]['passwordHash']
